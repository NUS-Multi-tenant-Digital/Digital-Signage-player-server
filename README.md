...
- 阿里云 OSS + CDN（媒体资源存储与分发）
- device_token 鉴权（Bearer token 查库验证）
...
```bash
mysql -u root -e "CREATE DATABASE IF NOT EXISTS digital_signage DEFAULT CHARACTER SET utf8mb4"
mysql -u root digital_signage < sql/schema.sql
mysql -u root digital_signage < sql/mvp_seed.sql
```
...
1. 设备首次启动 → 调用 `register` 获取 `device_token`
